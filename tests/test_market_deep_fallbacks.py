    ...
    async def get(self, url, headers=None, timeout=10, params=None):
        ...
        if self.scenario == "cmc_ok" and "coinmarketcap" in url:
            return DummyResp(
                ...
                {
                    "status": {"timestamp": "t"},
                    "data": {"BTC": {"quote": {"USD": {"price": 101, "volume_24h": 11, "market_cap": 1100}}}},
                },
            )
...
def test_market_cache_hit(monkeypatch):
    ...
    market_mod.cache.set(
        "market_bitcoin", {"symbol": "bitcoin", "price": 1, "volume_24h": 0, "marketcap": 0, "dominance": 1}
    )
    # should_skip False
...
def test_market_primary_keyerror_to_schema_then_fallback_success(monkeypatch):
    ...
    class BadResp(DummyResp):
        def json(self):
            return {
                "bad": "structure"
            }  # no market_data -> keys access yield empty values but not KeyError, so force KeyError by raising
    ...
    def cmc_get(url, timeout=10):
        return DummyResp(
            200, {"data": {"BITCOIN": {"quote": {"USD": {"price": 123, "volume_24h": 1, "market_cap": 10}}}}}
        )
