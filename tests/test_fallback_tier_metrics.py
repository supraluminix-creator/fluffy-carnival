...
async def test_macro_tier_metric(monkeypatch):
    ...
    for metric in coll.collect():
        for s in metric.samples:
            if s.labels.get('collector') == 'macro' and s.labels.get('tier') == '2' and s.labels.get('status') == 'success':
                found = True
...
async def test_deriv_oi_tier_metric(monkeypatch):
    ...
    for metric in coll.collect():
        for s in metric.samples:
            if s.labels.get('collector') == 'deriv_oi' and s.labels.get('tier') == '2' and s.labels.get('status') == 'success':
                found = True
