...
COLLECTOR_CALLS = {
    'market': lambda: __import__('pipeline.collectors.market', fromlist=['fetch_market']).fetch_market('bitcoin', cache_ttl=1),
    'defillama': lambda: asyncio.run(__import__('pipeline.collectors.defillama', fromlist=['get_chain_data']).get_chain_data('Ethereum')),
    'binance_spot': lambda: __import__('pipeline.collectors.binance', fromlist=['fetch_binance_spot_price']).fetch_binance_spot_price('BTCUSDT'),
    'binance_oi': lambda: __import__('pipeline.collectors.binance', fromlist=['fetch_binance_futures_oi']).fetch_binance_futures_oi('BTCUSDT'),
    'binance_funding': lambda: __import__('pipeline.collectors.binance', fromlist=['fetch_binance_funding']).fetch_binance_funding('BTCUSDT'),
    'deriv_funding': lambda: asyncio.run(__import__('pipeline.collectors.derivatives', fromlist=['fetch_bybit_funding']).fetch_bybit_funding('BTCUSDT')),
    'deriv_lsr': lambda: asyncio.run(__import__('pipeline.collectors.derivatives', fromlist=['fetch_bybit_long_short_ratio']).fetch_bybit_long_short_ratio('BTCUSDT')),
    'sentiment': lambda: asyncio.run(__import__('pipeline.collectors.sentiment', fromlist=['fetch_fear_greed']).fetch_fear_greed(cache_ttl=1)),
    'onchain_txcount': lambda: __import__('pipeline.collectors.txcount', fromlist=['TxCountCollector']).TxCountCollector().fetch_txcount('BTC'),
    'onchain_hashrate': lambda: __import__('pipeline.collectors.hashrate', fromlist=['HashrateCollector']).HashrateCollector().fetch_hashrate('BTC'),
