...
class DummyResp:
    ...
    def raise_for_status(self):
        if self.status_code >= 400:
            ...
            raise httpx.HTTPStatusError("err", request=self._request, response=response)
...
async def test_deriv_oi_primary_upstream_fallback_binance(monkeypatch):
    ...
    bybit_req = httpx.Request("GET", "https://api.bybit.com/")
    bybit_exc = httpx.HTTPStatusError("bad", request=bybit_req, response=httpx.Response(502, request=bybit_req))
...
async def test_deriv_oi_primary_rate_limit_metric(monkeypatch):
    ...
    rate_req = httpx.Request("GET", "https://api.bybit.com/")
    rate_exc = httpx.HTTPStatusError("rl", request=rate_req, response=httpx.Response(429, request=rate_req))
...
async def test_deriv_funding_fallback_binance(monkeypatch):
    ...
    funding_req = httpx.Request("GET", "https://api.bybit.com/")
    funding_exc = httpx.HTTPStatusError("fund", request=funding_req, response=httpx.Response(500, request=funding_req))
