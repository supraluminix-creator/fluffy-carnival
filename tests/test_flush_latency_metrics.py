...
def test_flush_latency_success(tmp_path, monkeypatch):
    ...
    writer = BybitLiquidationsWriter(
        db=str(db_path), parquet_dir=str(tmp_path / "pq"), flush_size=2, parquet_enabled=False
    )
    events = [
    ...
    assert any(
        s.labels.get("writer") == "bybit_liq" and s.labels.get("status") == "success" for s in samples
    )
...
def test_flush_latency_failure(tmp_path, monkeypatch):
    ...
    writer = BybitLiquidationsWriter(
        db=str(db_path), parquet_dir=str(tmp_path / "pq"), flush_size=1, parquet_enabled=True
    )
    ...
    async def push():
        await writer.write_record(
            {"symbol": "BTCUSDT", "side": "Sell", "price": 10100, "size": 1, "updatedTime": 1700000100000}
        )
        await writer.flush()
    asyncio.run(push())
    failure_samples = _get_samples('flush_failures_total')
    assert any(
        s.labels.get("writer") == "bybit_liq" and s.labels.get("phase") == "write" and s.value >= 1
        for s in failure_samples
    )
    latency_samples = _get_samples('writer_flush_latency_seconds')
    assert any(
        s.labels.get("writer") == "bybit_liq" and s.labels.get("status") == "error" for s in latency_samples
    )
