...
def test_fetch_market_primary_success(monkeypatch):
    data = {"market_data": {"current_price": {"usd": 100}, "total_volume": {"usd": 2000}, "market_cap": {"usd": 30000}, "market_cap_rank": 5}}
...
def test_fetch_market_fallback_cmc(monkeypatch):
    ...
    def fake_get(url, timeout=10):
        if 'coingecko' in url:
            raise httpx.HTTPStatusError('cg', request=SimpleNamespace(url='u'), response=SimpleNamespace(status_code=500))
        # CMC path
        return DummyResp({"data": {"BITCOIN": {"quote": {"USD": {"price": 101, "volume_24h": 2100, "market_cap": 31000, "market_cap_dominance": 40}}}}})
    monkeypatch.setattr(mmod, 'httpx', SimpleNamespace(get=fake_get))
    rec = mmod.fetch_market('bitcoin')
...
async def test_fetch_macro_primary_success(monkeypatch):
    async def fake_get(url, timeout=10):  # used for _async_http_get_json wrapper
        return DummyResp({"market_data": {"current_price": {"usd": 150}, "total_volume": {"usd": 5000}, "market_cap": {"usd": 80000}, "market_cap_rank": 2}, "last_updated": "ts"})
    class FakeAsyncClient:
        async def __aenter__(self): return self
        async def __aexit__(self, exc_type, exc, tb): return False
        async def get(self, url, timeout=10): return DummyResp({"market_data": {"current_price": {"usd": 150}, "total_volume": {"usd": 5000}, "market_cap": {"usd": 80000}, "market_cap_rank": 2}, "last_updated": "ts"})
...
async def test_fetch_macro_spot_fallback(monkeypatch):
    ...
    def fake_spot(sym):
        return {"timestamp": 0, "asset": "BTC", "symbol": sym, "metric_name": "spot_price_usdt", "value": 123.0, "source": "binance_spot", "confidence_score": 0.9}
...
async def test_fetch_macro_cmc_fallback(monkeypatch):
    ...
    class FakeAsyncClient2:  # CMC success
        ...
        async def get(self, url, headers=None, timeout=10):
            return DummyResp({"data": {"BITCOIN": {"quote": {"USD": {"price": 160, "volume_24h": 6000, "market_cap": 90000, "market_cap_dominance": 55}}}}, "status": {"timestamp": "t2"}})
    # Sequence: CG then CMC
    clients = [FakeAsyncClient1(), FakeAsyncClient2()]
    def next_client(): return clients.pop(0)
