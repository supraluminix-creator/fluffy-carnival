    ...
    for fam in REGISTRY.collect():
        if fam.name == family_name:
            for s in fam.samples:
                if (
                    s.name == f"{family_name}_total"
                    and s.labels.get('collector') == collector
...
def test_instrumentation_success_path():
    ...
    assert any(
        s.name == 'collector_duration_seconds_count'
        and s.labels.get('collector') == 'unit_demo_success'
        for fam in REGISTRY.collect()
        for s in fam.samples
    )
    ...
    def _error_counter_value():
        for fam in REGISTRY.collect():
            if fam.name == 'collector_error_types':
                for s in fam.samples:
                    if (
                        s.name == 'collector_error_types_total'
                        and s.labels.get('collector') == 'unit_demo_error'
                    ):
                        return s.value
        return 0.0
