...
def test_binance_spot_missing_price(monkeypatch):
    ...
    monkeypatch.setattr(bmod, "httpx", SimpleNamespace(get=fake_get))
    before = (
        COLLECTOR_ERROR_TYPES_TOTAL.labels(collector="binance_spot", error_type="schema")._value.get()
        if ("binance_spot", "schema") in COLLECTOR_ERROR_TYPES_TOTAL._metrics
        else 0
    )
...
def test_binance_futures_oi_upstream(monkeypatch):
    ...
    monkeypatch.setattr(bmod, "httpx", SimpleNamespace(get=fake_get))
    before = (
        COLLECTOR_ERROR_TYPES_TOTAL.labels(collector="binance_oi", error_type="upstream")._value.get()
        if ("binance_oi", "upstream") in COLLECTOR_ERROR_TYPES_TOTAL._metrics
        else 0
    )
    rec = bmod.fetch_binance_futures_oi("BTCUSDT")
    assert rec is None
