...
async def test_perform_export_batch_success(monkeypatch, tmpdir_fs):
    ...
    async def fake_oi(*a, **k):
        return rec("bybit_oi", 5)

    async def fake_lsr(*a, **k):
        return rec("bybit_lsr", 6)

    async def fake_defi(*a, **k):
        return rec("defillama", 7)

    async def fake_sent(*a, **k):
        return rec("sentiment", 8)
