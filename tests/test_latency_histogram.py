...
async def test_macro_latency_histogram(monkeypatch):
    ...
    def fake_httpx_get(url, headers=None, timeout=10):
        return DummyResp({
            ...
            'data': {'BITCOIN': {'quote': {'USD': {'price': 60000, 'volume_24h': 1500, 'market_cap': 920000000, 'market_cap_dominance': 57.0}}}}
        })
    ...
    for metric in coll.collect():
        for s in metric.samples:
            if s.name.endswith('_bucket') and s.labels.get('collector') == 'macro' and s.labels.get('tier') == '2' and s.labels.get('status') == 'success' and s.value > 0:
                found = True
...
async def test_deriv_oi_latency_histogram(monkeypatch):
    ...
    for metric in coll.collect():
        for s in metric.samples:
            if s.name.endswith('_bucket') and s.labels.get('collector') == 'deriv_oi' and s.labels.get('tier') == '2' and s.labels.get('status') == 'success' and s.value > 0:
                found = True
