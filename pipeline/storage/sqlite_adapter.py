...
_ENV_DB_PATH = "CRYPTO_DB_PATH"


def get_default_db_path() -> str:
    """Retourne le chemin DB principal en respectant l'override env.

    Le flag `CRYPTO_DB_PATH` permet de relocaliser la base (ex: SSD D:). On
    tolère les chemins relatifs pour compat legacy, sinon on résout en absolu
    pour éviter les surprises lors des jonctions NTFS.
    """

    env_override = os.getenv(_ENV_DB_PATH)
    if not env_override:
        return "data/crypto.db"
    return env_override
...
def get_connection(path: str | None = None, *, readonly: bool = False) -> sqlite3.Connection:
...
__all__ = ["get_connection", "transaction", "get_default_db_path", "ensure_db_parent"]
