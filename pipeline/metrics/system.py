...
HEARTBEAT_TICKS_TOTAL = _counter(
	...
	"Nombre de ticks heartbeat", ["source"],
)
HEALTH_REQUESTS_TOTAL = _counter(
	"health_requests_total",
	"Requêtes sur endpoints health", ["endpoint", "status"],
)
...
MAINTENANCE_CYCLES_TOTAL = _counter(
	"maintenance_cycles_total", "Cycles maintenance exécutés (success/error)", ["status"],
)
