...
class Reporter:
    ...
    def display_summary(
        ...
        mc = data.get("global", {})
        v_mc_ratio = (
            mc.get("total_volume", {}).get("usd", 0) / mc.get("total_market_cap", {}).get("usd", 1)
            if mc.get("total_market_cap", {}).get("usd", 0) > 0
            else 0
        )
        print(
            f"🌍 MACRO: Cap ${mc.get('total_market_cap',{}).get('usd',0)/1e12:.2f}T | Vol ${mc.get('total_volume',{}).get('usd',0)/1e9:.1f}B",
            file=stream,
        )
        print(
            f"📈 BTC Dominance: {mc.get('market_cap_percentage',{}).get('btc','N/A'):.1f}% | 24h Change: {mc.get('market_cap_change_percentage_24h_usd','N/A'):+.2f}%",
            file=stream,
        )
        ...
        if additional_data and "stablecoins" in additional_data:
            ...
            print(
                f"💵 Stablecoins: ${sc_data.get('total_supply',0)/1e9:.1f}B (USDT: {sc_data.get('usdt_dominance','N/A'):.1f}%, USDC: {sc_data.get('usdc_dominance','N/A'):.1f}%)",
                file=stream,
            )
        ...
        for coin_id, info in data.get("coins", {}).items():
            ...
            if symbol:
                ...
                v_mc_ratio = (
                    info.get("usd_24h_vol", 0) / info.get("usd_market_cap", 1)
                    if info.get("usd_market_cap", 0) > 0
                    else 0
                )
                row = [
                    symbol,
                ...
                if symbol in ["BTC", "ETH"]:
                    ...
                    if onchain_data:
                        row.append(
                            f"TXs={onchain_data.get('transaction_count','N/A')}, HashRate={onchain_data.get('hash_rate','N/A')}"
                        )
                    deriv_data = additional_data.get("derivatives", {}).get(symbol, {}) if additional_data else {}
                    if deriv_data:
                        row.append(
                            f"FR={deriv_data.get('funding_rate','N/A')}, OI={deriv_data.get('open_interest','N/A')}"
                        )
                rows.append(row)
        ...
        if signals:
            print("\n🚦 SIGNALS:", file=stream)
            print(
                f"   Accumulation: {self.get_signal_emoji(signals.get('accumulation_signal', 0))} {signals.get('accumulation_signal', 0)}",
                file=stream,
            )
            print(
                f"   Levier: {self.get_signal_emoji(signals.get('leverage_signal', 0))} {signals.get('leverage_signal', 0)}",
                file=stream,
            )
            print(
                f"   Liquidité: {self.get_signal_emoji(signals.get('liquidity_signal', 0))} {signals.get('liquidity_signal', 0)}",
                file=stream,
            )
        ...
        if fgi_data:
            sentiment_icon = (
                "😨" if int(fgi_data.get("value", 50)) < 25 else "😊" if int(fgi_data.get("value", 50)) > 75 else "😐"
            )
            print(
                f"\n🎭 SENTIMENT: {fgi_data.get('value', 'N/A')} {sentiment_icon} ({fgi_data.get('value_classification', 'N/A')})",
                file=stream,
            )
